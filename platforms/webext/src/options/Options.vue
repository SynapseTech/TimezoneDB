...
<script setup lang="ts">
	...
	onMounted(async () => {
		...
		timeFormat.value = extensionSettings.timeFormat;
	});

	async function save() {
		let extensionSettings: ExtensionSettings =
			(await browser.storage.sync.get('settings')) as ExtensionSettings;
		extensionSettings.apiUrl = apiUrl.value;
		extensionSettings.timeFormat = timeFormat.value;
